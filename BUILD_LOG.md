...
### webhook_events.received_at did not mean "received" (2026-09-21 21:55)
- Symptom: while checking Bob's real payment, the webhook_events row showed received_at 20:34:20.245 and processed_at 20:34:20.244: the request looked as if it had been received one millisecond AFTER it had finished being processed. The owner's question was whether the column measured what its name said.
- Investigation: searched every use of the column (only the schema and the first migration mention it; no application code reads it), then compared the two timestamps on all 3 real rows: received_at was 6, 4 and 1 ms after processed_at, never before. Traced why: the row is only inserted at the END of processing (deliberately, inside the fulfilment transaction, so a crash leaves no half-claimed event), and received_at took its value from the column default, which Prisma fills in from the application clock when the insert is built. So it held the time the row was written. It also could not be the true arrival time for a second reason: nothing ever recorded the arrival time at all. The real arrival was only visible in ngrok's request history (21:34:19 local, from Paystack's address, HTTP 200).
- Cause: my own naming and design drift. I named the column in the first schema for a claim-first design (insert a row the moment the event arrives, then process, so insert time equals arrival time). When I later built the handler the other way round (insert only after processing, in one transaction), I never revisited the name, and no test asserted anything about the column, so nothing caught it. The comment on the neighbouring processed_at column still described the old claim-first meaning too. I also reused a general habit of trusting a column default without checking whose clock and which moment it captures.
- Fix: option C, chosen by the owner after I laid out the trade-offs: the handler stamps the arrival time on its very first line and passes it in, so the name is now true. No migration and no rename. The three existing rows keep their old values on purpose and are documented as "about processed_at". New tests: arrival never later than processing, a visible gap under a simulated 400 ms verify call, the stamp precedes a slow request body, a redelivery keeps the first arrival, and an invariant over every event recorded in a run (which flags exactly the 3 old real rows, showing it detects the old behaviour). See DECISIONS.md, "webhook_events.received_at is the true arrival time".
- Commit: f5eec90

### Lint flagged my use of window.location.assign for an internal redirect (2026-09-22 00:45)
- Symptom: after building the plans and billing pages, `npm run lint` reported 2 warnings: "Do not use `window.location.assign()` to navigate to internal Next.js pages. Use `redirect()` in the render phase, or `useRouter().push()` in Client Components' event handlers instead" (in CheckoutButton.tsx and CancelPlan.tsx).
- Investigation: the warnings were about the sign-in redirect on a 401, where I had reached for a full-page navigation. The Paystack redirect is different: it must leave the app entirely, so it needs a real page navigation, not the router. Typecheck had passed, so this was style and correctness of navigation, not a type error.
...
### My own cancellation test used the wrong database client throughout, then the wrong one again for the HTTP section (2026-09-23 03:10)
- Symptom: the first run of check-cancellation.ts had 21 of 62 checks fail, spanning direct calls to cancelSubscription() that should have succeeded (e.g. "active, in-period -> outcome canceled" failed) and every real-HTTP check past the 401/403 pair.
- Investigation: (1) isolated a single direct call (cancelSubscription(u.id, "reason") with NO client argument) in a throwaway script and it returned { outcome: 'canceled', ... } correctly when the isolated schema's client WAS passed explicitly. The bug: cancelSubscription/resumeSubscription default their `client` parameter to the app's real `db` singleton (imported from @/lib/db), and my test script called them without that third argument almost everywhere, so every "logic" test was silently running against the real public schema, where the throwaway user (created via the isolated schema's client) does not exist -- findUnique returned null, describeSubscription(null) returned Free, and every outcome came back not_subscribed/not_canceled instead of the real result. Nothing crashed, so it looked like the feature was broken rather than the test. (2) Fixing that broke nothing new, but every "real HTTP" check still failed for a different reason: I had reused the SAME `db` variable (the isolated schema's client) to create the throwaway user and session for the HTTP section too, but the live dev server (a separate OS process) only ever queries the real public schema, so it saw no session at all -- checks expecting 400 got 401 instead, etc. Compared against check-billing.ts, which correctly imports the real `db` directly for its HTTP-facing section and never touches the isolated-schema harness at all.
- Cause: my own test script design, in two separate ways: forgetting a required parameter in direct calls, and conflating "a client that talks to an isolated schema" with "the client the live app itself uses" for the HTTP section. Neither is a defect in cancelSubscription/resumeSubscription or the routes.
- Fix: rewrote the script with two explicitly separate handles: `tdb` (the isolated schema, for Part 1's direct calls, passed to every call) and `realDb` (imported straight from @/lib/db, for Part 2's throwaway user/session/subscription against the live app, matching check-billing.ts's pattern). Rerun: 42 of 42 pass.
- Commit: fa01637 (code was already correct), a3af15a (the fixed test)

### Building the real cancel endpoint broke check-billing.ts's tests of the old placeholder (2026-09-23 03:25)
- Symptom: after replacing the 501 placeholder with the real logic, `npm run check:billing` regressed: 5 failures, all either asserting a 501/NOT_IMPLEMENTED response that no longer happens, or asserting the pre-cancellation-feature billing-page wording ("Your plan will end on <date>." with no Keep my plan control).
- Investigation: these assertions were written intentionally, in the billing-UI task, to pin down the PLACEHOLDER's contract (still 501, still validates its body) since the real behaviour did not exist yet. They were never meant to survive the placeholder being replaced.
- Cause: expected, not a bug -- flagged here per the append-only build log convention rather than silently edited away.
- Fix: removed check-billing.ts's placeholder-endpoint block entirely (that surface is now fully covered, with real behaviour, by check:cancellation) and updated its one billing-page wording assertion to the real copy ("...and won't renew.", plus the Keep my plan control). Rerun: check:billing 51/51, check:cancellation 42/42.
- Commit: a3af15a
