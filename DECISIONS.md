...
### webhook_events.received_at is the true arrival time (a rename and a comment-only fix were both rejected)
- Decision: what to do about a column named received_at that did not hold when the request was received.
- Chosen: make the name TRUE, with no rename and no migration. The webhook handler stamps the arrival time on its very first line, before the body is read, before the signature is checked and before any Paystack call, and passes it into the event row (WebhookEventInput.receivedAt). Because it and processed_at come from the same (application) clock, received_at <= processed_at always, and the gap between them is our own processing time: reading the body, verifying with Paystack and writing. It is recorded for every outcome (fulfilled, already_fulfilled, amount_mismatch, verification_failed, unknown_tx_ref). A redelivery never overwrites the first arrival time, because a repeated delivery creates no new row. The schema comment explains it, including the history below.
- Rejected and why: renaming the column to created_at or recorded_at (an honest name, but it would leave two near-identical timestamps that never differ by more than a few milliseconds, and the true arrival time would still not be recorded anywhere; the arrival time is real evidence, for example that a webhook reached us after the return page had already fulfilled the payment); a comment-only fix (the misleading name would remain, which is exactly how I misread the column myself); adding a CHECK (received_at <= processed_at) (it would fail on the three existing rows, and the tests assert the invariant instead).
- The three existing real rows keep their old values on purpose: they were written before the fix, when the column simply took the time the row was written, so their received_at is 1 to 6 ms AFTER processed_at (payment 7aba513…: 6 ms, 92769fe…: 4 ms, 6c584b1…: 1 ms), which is impossible for an arrival time. Read them as "about processed_at". They are real evidence (and the invariant check in the tests flags exactly these three, which shows it detects the old behaviour). Their real arrival times are in ngrok's request history for the last one (20:34:19 UTC) and in the payment timeline in docs/evidence/subscription-extension.md.
- Tested: the stored value equals the value passed in exactly; a 400 ms Paystack verify shows a visible gap (measured 411 ms); a request body that takes 350 ms to arrive still gets an arrival stamp within 0 ms of sending, proving the stamp precedes the body read; a redelivery leaves the first arrival untouched; no row anywhere in a whole test run has received_at later than processed_at.
- Files: src/lib/paystack/webhook-handler.ts, src/lib/checkout/fulfil.ts, prisma/schema.prisma (comment only), scripts/check-webhook.ts, scripts/check-fulfilment.ts

### Plans and billing pages: minimal, and honest about what the system does
- Decision: how the plans page (/plans) and the billing page (/billing) behave.
- Chosen: one function, describeSubscription(), decides what plan a person is on, so the two pages can never disagree. A person is on Pro only while their subscription status is 'active' AND its period ends strictly after now; no row, an ended period, past_due and canceled are all the Free plan (the billing page says which). /plans shows Free, Pro (monthly) and Pro (yearly), with the current plan marked (aria-current), prices from the plan config, and a button per paid option that calls the existing POST /api/checkout (the browser sends only the interval, never an amount) and then goes to the Paystack page it returns; the returned URL is checked again in the browser (https on paystack.com) before it is followed. /billing shows the plan, its status and its end date, or "You're on the Free plan.", plus a Cancel control (a button, a confirmation step and an optional reason of up to 500 characters). Both pages are behind sign-in, are never cached, and are linked from the dashboard by two plain links. The return page's "Try again" now goes to /plans (it was a temporary /dashboard).
- Two things differ from what one might expect, on purpose. (1) For someone with an ACTIVE plan the other paid option is not a button: POST /api/checkout answers 409 while a plan is active (approved earlier, so nobody pays twice by accident), so the page says "Available when your current plan ends on <date>" instead of offering a button that would fail. (2) The billing page says "Active until <date>", never "Renews on": there is no automatic renewal, each period is a separate payment, and "renews" would be untrue.
- The Cancel control is wired to a PLACEHOLDER, POST /api/subscription/cancel, which after the same-origin and sign-in checks and body validation answers 501 NOT_IMPLEMENTED; the UI then says "Cancelling a plan isn't available yet." The cancellation itself is the next task. What is fixed now is the contract that task fills in: only a signed-in person, same-origin, and an optional reason of 1 to 500 characters (the database CHECK on cancellation_reason). The Cancel button is shown only for an active Pro plan that is not already set to end.
...
### Cancellation and resume: no cron, no early status flip, eligibility derived from describeSubscription()
- Decision: how cancelling a plan is implemented, given the brief requires access retained until the period ends and this project has no background job.
- Chosen: cancelSubscription() only ever sets cancel_at_period_end = true and stores the optional reason (1-500 chars, the existing CHECK); status stays 'active' for the whole remaining period. No job or webhook ever flips it to 'canceled' or off: describeSubscription() already compares current_period_end to now() on every call (built for the plans/billing pages), so the same read-time check that already reports Free once a period lapses does so for a cancelled row too, with zero new code. resumeSubscription() is the exact inverse (cancel_at_period_end = false, reason cleared in the SAME write -- the CHECK on cancellation_reason requires cancel_at_period_end or status='canceled', so it can never be left orphaned) and is only reachable while still in-period. Both functions derive eligibility from describeSubscription() itself (cancel: kind==='pro' && !endsAtPeriodEnd; resume: kind==='pro' && endsAtPeriodEnd), so the rule the API enforces and the rule the pages render from can never drift apart.
- status = 'canceled' is a RESERVED, currently-unwritten value. The CHECK constraint and describeSubscription() both already handle it (treated as Free, same as past_due), but nothing in this feature -- or anywhere else in the app -- ever writes it. It is left for a possible future admin action (an immediate revoke, as opposed to end-of-period cancellation), and documented here so it does not read as an oversight.
- Un-cancel (resume) was not explicitly requested by the brief but was approved as in-scope: a two-column, no-migration addition, and leaving someone stuck after one click with no way back except waiting out the period was judged worse than the small addition.
- No rate limiting on either endpoint (same reasoning as sign-in, and unlike checkout): no external API call, no money movement, and the worst outcome of hammering it is flipping a boolean back and forth, which resume/cancel already do symmetrically. Same-origin plus the session requirement is the only defence, and is enough.
- Rejected and why: flipping status to 'canceled' immediately (contradicts "access retained until the period ends"); a cron/scheduled job to expire cancelled subscriptions (unnecessary -- the read-time check already does this for every other "has the period ended" case); storing the cancellation timestamp separately from cancel_at_period_end (nothing reads it; the period end itself is the only date that matters here); rate limiting cancel/resume (no abuse path exists for a same-origin, session-gated boolean flip with no side effects).
- Files: src/lib/billing/cancel.ts, src/app/api/subscription/cancel/route.ts, src/app/api/subscription/resume/route.ts, src/app/billing/{page,CancelPlan,ResumePlan}.tsx

### Upgrade with proration (Pro monthly -> Pro yearly, mid-cycle)
- Decision: how a mid-cycle upgrade is priced, how it moves through the payment pipeline, and what happens to the subscription's period when it is paid.
- Chosen formula (src/lib/billing/proration.ts): unused_fraction = (current_period_end - now) / (current_period_end - current_period_start); credit_kobo = round(unused_fraction * monthly_price_kobo); charge_kobo = yearly_price_kobo - credit_kobo, floored at 1 kobo. "Unused" is measured against the period's OWN real length, never an assumed 30 days, because Postgres month-end clamping (31 Jan + 1 month = 28 Feb) means a monthly period is not always 30 days. Worked example (day 12 of a 30-day cycle, 18 days unused): credit 180,000 kobo (NGN 1,800), charge 2,820,000 kobo (NGN 28,200).
- Reuses the checkout->verify->fulfil pipeline almost entirely, as the owner suggested: initiateUpgrade() shares startPaystackCheckout() (factored out of initiateCheckout()) to write the 'initiated' row and call Paystack -- the only difference from a normal purchase is the amount (the prorated charge, not the fixed yearly price) and that it is allowed while a monthly subscription is already active. From Paystack's initialize call onward -- verify, evaluateTransaction, fulfilTransaction, the webhook, the return page -- an upgrade is indistinguishable from any other yearly purchase; no new fulfilment code path, no new webhook handling.
- What happens to the period on fulfilment: it REPLACES the old one and starts today, rather than stacking a year on top of whatever monthly time was left (stacking would double-count the unused time already folded into the credit). This required a real behaviour change to activateSubscription's SQL: the existing "stack while active" branch now also requires the billing interval to be UNCHANGED. Same interval while active (a renewal, e.g. bob's two monthly payments) still stacks exactly as before; a DIFFERENT interval while active (an upgrade) now replaces. This is a general rule, not upgrade-specific plumbing, and needed no schema change -- one added condition in the existing CASE expression.
- Where triggered: an "Upgrade to yearly" control on /billing, shown only when Pro, monthly, active, and NOT cancel_at_period_end. The price is computed server-side by quoteUpgrade() both to display it (billing/page.tsx) and to charge it (POST /api/subscription/upgrade, fresh from the database at the moment of the call) -- the price shown is never itself sent back to the server.
- Upgrading a subscription set to cancel is blocked (PLAN_ENDING): the owner approved requiring "Keep my plan" (resume) first rather than having upgrade silently also un-cancel, since combining the two in one action is harder to reason about and to explain to a reviewer than two separate, already-existing actions.
- Rejected and why: assuming a flat 30-day month for the proration fraction (wrong whenever the period is not 30 days, e.g. February); letting upgrade also implicitly resume a cancelling subscription (conflates two decisions into one click); a new fulfilment code path for upgrades (unnecessary -- the existing pipeline already handles "a payment for a different amount/interval than the plan's list price" once the amount comes from the 'initiated' row, which it always did); stacking the new year on top of the remaining monthly period (double-counts the credit already priced in).
- Files: src/lib/billing/proration.ts, src/lib/checkout/initiate.ts, src/lib/checkout/fulfil.ts, src/app/api/subscription/upgrade/route.ts, src/app/billing/{page,UpgradeToYearly}.tsx, scripts/check-upgrade.ts, scripts/check-fulfilment.ts (one pre-existing test updated -- see BUILD_LOG.md)
...
- Automatic renewal: not built (each period is a separate payment; see the checkout decisions). Cancellation/resume and upgrade-with-proration, listed here as not built when this line was first written, were both built since -- see the "Cancellation and resume" and "Upgrade with proration" decisions above. A downgrade (yearly -> monthly, or Pro -> Free before the period ends) remains not built: not requested, and "Free before the period ends" would mean forfeiting money already paid, which the brief's refund/dispute exclusion (below) already puts out of scope.
